...
def char_embedding(inputs, voca_size, embedding_dim, length, charMaxLen, initializer=None, reuse=False, trainable=True,
                   scope='EmbeddingChar'):
    ...
    if initializer == None:
        initializer = np.concatenate((np.zeros((1, embedding_dim), dtype='float32'),  #Padding : [1,dim]
                                      np.random.rand(voca_size - 1, embedding_dim).astype('float32')),
                                     axis=0)  #Other than Padding [vocasize-1,dim]
    else:
        initializer = np.concatenate((np.zeros((1, embedding_dim), dtype='float32'),  #Padding [1,dim]
