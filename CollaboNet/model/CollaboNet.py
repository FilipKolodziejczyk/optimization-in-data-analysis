...
class Model:
            ...
            with tf.compat.v1.variable_scope(args.guidee_data+"CLWE_CNN"):
                for fs in filter_size:  
                    with tf.compat.v1.variable_scope("clwe_CNN-%s" % fs):
                        ...
                        pool_flat_d = tf.nn.dropout(x=pool_flat, rate=1 - (1-self.emb_dropout), name='clwe_drop', seed=self.seed)
    ...
    def model(self, args, X_embedded_data, X_embedded_char, guideeInfo, summery, scopename='main'):
        ...
        with tf.compat.v1.variable_scope(scopename) as scope:
            ...
            outputs_fw_t = tf.nn.dropout(x=self.outputs_fw, rate=1 - (1-self.lstm_dropout), name='output_drop_fw', seed=self.seed)
            outputs_bw_t = tf.nn.dropout(x=self.outputs_bw, rate=1 - (1-self.lstm_dropout), name='output_drop_bw', seed=self.seed)
            ...
            self.sequence_loss = tfa.seq2seq.sequence_loss(
            ...
            if args.tensorboard:
                ...
                self.variable_summaries(self.outputs_concat)
    ...
    def variable_summaries(self, var): #Attach a lot of summaries to a Tensor (for TensorBoard visualization).
        with tf.compat.v1.name_scope('VarSummaries'):
